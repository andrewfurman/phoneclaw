...
const details = await waitForConversationTranscript(conversation.conversationId);
const archive = await workerJson("/conversation-history/archive-elevenlabs", {
  conversation_id: conversation.conversationId,
});
const archived = await workerJson("/conversation-history/get", {
  conversation_id: conversation.conversationId,
});

const checks = {
  conversation_id_returned: Boolean(conversation.conversationId),
  elevenlabs_transcript_available: Array.isArray(details.transcript) && details.transcript.length > 0,
  archive_endpoint_ok: archive.ok === true,
  conversation_history_get_ok: archived.ok === true,
  archived_transcript_available:
    Array.isArray(archived.conversation?.transcript) &&
    archived.conversation.transcript.length > 0,
};
  ...
  JSON.stringify(
    {
      ...
      archived_started_at: archived.conversation?.started_at || null,
      archived_summary: archived.conversation?.summary || "",
      checks,
    },
    null,
    2
...
async function generateQuestionAudio(text) {
  const directory = await mkdtemp(join(tmpdir(), "phoneclaw-logging-audio-"));
...
async function runAudioConversation(ulawAudio) {
  const signed = await elevenLabsJson(
  ...
  ws.addEventListener("message", (event) => {
    ...
    if (message.type === "agent_response") {
      ...
      if (streamed && isRealAgentMessage(text)) settle();
      return;
    }
...
async function waitForConversationTranscript(conversationId) {
  const deadline = Date.now() + 45_000;
  let last;

  while (Date.now() < deadline) {
    last = await elevenLabsJson(
      `${apiBase}/v1/convai/conversations/${encodeURIComponent(conversationId)}`
    );
    if (Array.isArray(last.transcript) && last.transcript.length > 0) return last;
    await wait(1_500);
  }
...
async function elevenLabsJson(url) {
