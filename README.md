...
| Tavily | Preferred LLM-oriented web search provider for fast, compact voice answers. | Optional `TAVILY_API_KEY` Worker secret. With `WEB_SEARCH_PROVIDER=auto`, the Worker uses Tavily when the key exists and falls back if it fails or returns no useful results. |
| DuckDuckGo | No-key fallback web search provider. | No secret required. Used when Tavily is unavailable, fails, or returns no useful results. |
| Wikipedia | Final no-key factual fallback for basic general-knowledge queries when primary search providers return no useful results. | No secret required. Used only after the primary web-search path has no useful result. |
...
`himalaya_email_list` is paginated by default and returns compact envelope metadata only: id, subject, sender, recipients, date, flags, and attachment presence. Use `all_pages=true` on the same tool for complete folder lists and total-count questions such as "how many emails are in my inbox?" All-pages mode returns at most 200 envelopes by default and reports `has_more`, `complete`, and `capped` so the agent does not dump an entire mailbox into context. `himalaya_email_read` returns compact decoded headers and a bounded `body_text` excerpt by default; raw email source is opt-in through `include_raw=true` for explicit debugging requests only.
...
`rss_*` tools read configured RSS or Atom feeds from `RSS_FEEDS_CONFIG_PATH` or `RSS_FEEDS_JSON` on the private EC2 bridge. Each feed has an id, title, URL, optional request headers, privacy flag, and cache TTL. The bridge fetches feed XML only, parses `content:encoded`, Atom `content`, or summary fields, redacts private URLs in responses, and caches feed fetches to avoid repeated polling. Recent/search calls default to up to 200 entries and clamp at 1,000 entries, limited by however many entries the upstream feed actually emits. The agent is configured to use `10` for quick shortlists, `25` for spoken browsing, `100` for search scanning, `200` for high-default tool-side processing, and `1,000` only for explicit large retrieval or export requests. Results include `limit`, `max_limit`, `available_count`, and `has_more` so the agent can explain whether a shorter result came from the upstream feed or from PhoneClaw limiting. Store private feed URLs in `/etc/phoneclaw/rss-feeds.json` or another host-local secret file, not in Git or Worker config.
